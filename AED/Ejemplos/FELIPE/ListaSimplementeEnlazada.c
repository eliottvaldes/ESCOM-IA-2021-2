...
void AgregarPosN(Lista* lista, int *e, int posUsuario, Dato datousuario){
    struct Nodo* nodoUsuario=(struct Nodo*) malloc(sizeof(struct Nodo));
    struct Nodo* nodoCursor;
    int posicionFinal;
    if((*lista)->cabecera==NULL && posUsuario==0 ){
        agregarPorDerecha(lista, e,datousuario);
    }else{
        ...
        if (posUsuario>posicionFinal)
        {
            ...
            nodoCursor=(*lista)->cabecera;
            while(nodoCursor->pos!=posUsuario){
                nodoCursor=nodoCursor->siguiente;
            }
            nodoUsuario->dato=datousuario;
            nodoUsuario->pos=posUsuario;
            nodoUsuario->siguiente=nodoCursor->siguiente;
            //Ahora vamos a un nodo anterior
            nodoCursor=(*lista)->cabecera;
            while(nodoCursor->pos!=posUsuario-1){
                nodoCursor=nodoCursor->siguiente;
            }
           nodoCursor->siguiente=nodoUsuario;
           //AHORA SOLO AUMENTAMOS LAS POSICIONES PARATIR DEL NODO
           //NOS MOVEMOS DOS NODS ADELANTE
           nodoCursor=nodoCursor->siguiente;
           nodoCursor=nodoCursor->siguiente;
            while(nodoCursor->siguiente!=NULL){
                nodoCursor->pos++;
